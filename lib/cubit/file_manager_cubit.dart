...
import '../data/repostory/category_repostory.dart';

part 'file_manager_state.dart';

class FileManagerCubit extends Cubit<FileManagerState> {
  FileManagerCubit({required this.fileRepository})
      : super(
          FileManagerState(
            isDownloaded: false,
            progress: 0.0,
  ...
  Future<void> getAllFiles() async {
    ...
    try {
      var fileItem = await fileRepository.getAllFiles();
      emit(state.copyWith(
          status: FormzStatus.submissionSuccess, fileItem: fileItem));
    } catch (error) {
      print(error);
      emit(state.copyWith(
  ...
  void downloadIfExists({
    ...
    String newFileLocation =
        "${directory?.path}/${fileInfo.name}${DateTime.now().millisecond}${url.substring(url.length - 5, url.length)}";
    try {
      ...
      emit(
          state.copyWith(newFileLocation: newFileLocation, isDownloaded: true));
    } catch (error) {
